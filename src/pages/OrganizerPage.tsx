...
import { api, eventbriteModules, eventPlace, PageHero } from '../shared'
import type { Account, EventbriteModule, ModuleField, OrganizerEvent } from '../shared'
...
function ModuleCard({ module, eventId, account }: { module: EventbriteModule; eventId: string; account: Account | null }) {
...
export function OrganizerPage({ account }: { account: Account | null }) {
  ...
  useEffect(() => { if (!account) return; api<{ events: OrganizerEvent[] }>(`/dashboard/organizer?email=${encodeURIComponent(account.email)}`, { headers: account.apiToken ? { 'X-LightEvents-Token': account.apiToken } : undefined }).then(r => { setData(r.events); setSelectedEventId(String(r.events[0]?.event.id ?? '')) }).catch(() => setData([])) }, [account?.email])
  const organizerModules = useMemo(() => eventbriteModules, [])
  return <main><PageHero title="Tableau de bord organisateur" text="Événements, participants, réservations, statuts, pays et modules avancés type Eventbrite." /><section className="warning page">{account ? `Connecté: ${account.fullName}` : 'Connectez-vous comme organisateur.'}</section><section className="dashboard page">{data.map(row => <article className="panel" key={row.event.id}><h2>{row.event.title}</h2><p>{eventPlace(row.event)}</p><strong>⏳ {Math.floor(row.countdownSeconds / 86400)}j {Math.floor((row.countdownSeconds % 86400) / 3600)}h</strong><div className="stats"><span>{row.participants.length} participants</span><span>{row.reservations.length} réservations</span><span>{row.participants.filter(p => p.status === 'PAID').length} payés</span></div><h3>Pays</h3><ul>{Object.entries(row.participantCountries).map(([country, count]) => <li key={country}>{country}: {count}</li>)}</ul><h3>Participants</h3><div className="tableLike">{row.participants.map(p => <span key={p.id}>{p.fullName} · {p.email} · {p.status}</span>)}</div></article>)}</section><section className="page moduleHub"><div className="sectionTitleInline"><div><span className="eyebrow">Eventbrite-like</span><h2>Modules avancés organisateur</h2></div><p>Ces cartes préparent le front pour les prochains endpoints backend sous <code>/api</code>. Choisissez un événement, puis envoyez un payload minimal de test.</p></div><div className="moduleToolbar panel"><label>Événement cible<select value={selectedEventId} onChange={e => setSelectedEventId(e.target.value)}><option value="">Sélectionner un événement</option>{data.map(row => <option key={row.event.id} value={row.event.id}>{row.event.title} · #{row.event.id}</option>)}</select></label><span>{organizerModules.length} modules exposés</span></div><div className="moduleGrid">{organizerModules.map(module => <ModuleCard key={module.key} module={module} eventId={selectedEventId} account={account} />)}</div></section></main>
}
