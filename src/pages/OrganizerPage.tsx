...
export function OrganizerPage({ account }: { account: Account | null }) {
  ...
  useEffect(() => { if (!account) return; api<{ events: OrganizerEvent[] }>(`/dashboard/organizer?email=${encodeURIComponent(account.email)}`).then(r => { setData(r.events); setSelectedEventId(String(r.events[0]?.event.id ?? '')) }).catch(() => setData([])) }, [account?.email])
