...
export function HomePage({ events, loading, reload }: { events: EventItem[]; loading: boolean; reload: () => void }) {
  ...
  async function suggest(value: string){ setQuery(value); setCoords(null); setSelectedPlace(null); if(value.trim().length < 2) return setSuggestions([]); const apiSuggestions = await api<PlaceSuggestion[]>(`/geo/place-suggest?q=${encodeURIComponent(value)}`).catch(() => []); const local = Array.from(new Map(events.filter(e => e.city?.toLowerCase().includes(value.toLowerCase()) || e.country?.toLowerCase().includes(value.toLowerCase())).map(e => [`${e.city}-${e.country}`, { label: `${e.city}, ${e.country}`, city: e.city, country: e.country, countryCode: e.countryCode || '', latitude: e.latitude, longitude: e.longitude }])).values()); setSuggestions([...local, ...apiSuggestions].slice(0,8)) }
...
function HighlightEvent({ event }: { event: EventItem }) {
  ...
  return <article className="highlightCard"><div className="highlightImage"><img src={event.coverImageUrl || event.generatedImageUrl || '/favicon.svg'} /><div className="dateBadge"><b>{d.getDate()}</b><span>{d.toLocaleString('fr-FR', { month: 'short' })}</span></div></div><div className="highlightBody"><span className="highlightLabel">Highlight</span><h2>{event.title}</h2><small className="organizerMini">Par {event.organizerName || 'Organisateur LightEvents'}</small><p>{event.description}</p><div className="highlightMeta"><small>DATUM</small><b>{date(event.startsAt)}</b><span>{eventPlace(event)}</span></div><div className="highlightActions"><strong>{ticketPriceSummary(event.tickets)}</strong><button onClick={() => go(`/events/${event.id}`)}>Tickets sichern →</button></div></div></article>
}
